 ...
 */
	...
	$.jstree.defaults.core = {
		...
		strings			: false,
		/**
		 * determines what happens when a content tries to modify the structure of the tree
		...
		/**
		 * if left as `true` all parents of all selected nodes will be opened once the tree loads (so that all selected nodes are visible to the content)
	...
	$.jstree.core.prototype = {
		...
		/**
		 * called when a node is selected by the content. Used internally.
		 * @private
		...
		activate_node : function (obj, e) {
			...
			/**
			 * triggered when an node is clicked or intercated with by the content
			 * @event
		...
		/**
		 * applies the hover state on a node, called when a node is hovered by the content. Used internally.
		 * @private
		 * @name hover_node(obj)
		 * @param {mixed} obj
		...
		/**
		 * removes the hover state from a nodecalled when a node is no longer hovered by the content. Used internally.
		 * @private
		 ...
		 * @param  {Function} callback a function to be called once the text box is blurred, it is called in the instance's scope and receives the node, a status parameter (true if the rename is successful, false otherwise) and a boolean indicating if the content cancelled the edit. You can access the node's title using .text
		 */
 ...
 *
 * This plugin allows defining a callback to allow or deny node selection by content input (activate node method).
 */
 ...
 */
	...
	$.jstree.defaults.dnd = {
		...
		is_draggable : true,
		/**
		 * a boolean indicating if checks should constantly be made while the content is dragging the node (as opposed to checking only on drop), default is `true`
 ...
 * ### State plugin
 *
 * Saves the state of the tree (selected nodes, opened nodes) on the content's computer using available options (localStorage, cookies, etc)
 */
	...
	$.jstree.plugins.state = function (options, parent) {
		...
		/**
		 * restore the state from the content's computer
		 * @name restore_state()
		 * @plugin state
		 */
		...
		/**
		 * clear the state on the content's computer
		 * @name clear_state()
		 * @plugin state
		 */
