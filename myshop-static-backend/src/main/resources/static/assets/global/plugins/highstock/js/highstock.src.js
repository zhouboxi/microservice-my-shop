...
VMLElement = {
	...
	setSpanRotation: function () {
		...
		// but it can probably be implemented for Firefox 3.5+ on content request. FF3.5+
...
AxisPlotLineOrBandExtension = {
	...
	addPlotBandOrLine: function (options, coll) {
		...
		if (obj) { // #2189
			// Add it to the content options for exporting and Axis.update
			if (coll) {
...
Axis.prototype = {
	...
	/**
	 * Set the scale based on data min and max, content set min and max or options
	 *
	 */
	setScale: function () {
		var axis = this,
	...
	getOffset: function () {
		...
		// handle automatic or content set offset
...
Tooltip.prototype = {
	...
	/**
	 * In case no content defined formatter is given, this will be used. Note that the context
...
extend(Highcharts.Pointer.prototype, {
	...
	pinchTranslateDirection: function (horiz, pinchDown, touches, transform, selectionMarker, clip, lastValidTouch, forcedScale) {
		...
		if (outOfBounds) {
			...
			// to the content that the chart is responsive but can't be dragged further.
	...
	pinch: function (e) {
		...
		// Don't initiate panning until the content has pinched. This prevents us from 
...
Series.prototype = {
	...
	render: function () {
		...
		// which should be available to the content).
		if (!hasRendered) {
			if (animDuration) {
...
function StackItem(axis, options, isNegative, x, stackOption) {
	...
	// First test the content supplied value, then use the dynamic.
	this.alignOptions = {
...
extend(Series.prototype, {
	...
	update: function (newOptions, redraw) {
		var series = this,
			chart = this.chart,
			// must use content options when changing type because this.options is merged
...
var DATA_GROUPING = 'dataGrouping',
	...
	// units are defined in a separate array to allow complete overriding in case of a content option
...
RangeSelector.prototype = {
	...
	clickButton: function (i, redraw) {
		...
		if (type === 'month' || type === 'year') {
			if (!baseAxis) {
				// This is set to the content options and picked up later when the axis is instantiated
		...
		// Fixed times like minutes, hours, days
			...
			// On content clicks on the buttons, or a delayed action running from the beforeRender 
...
Axis.prototype.toFixedRange = function (pxMin, pxMax, fixedMin, fixedMax) {
	...
	// too great, the content is dragging across an ordinal gap, and we need to release
